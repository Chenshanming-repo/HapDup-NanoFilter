...
def run_hapcut2_filter_multiprocess(ref, bam, vcf, new_vcf, out_dir, only_snvs, rtype, threads_num, FILTER_SNP):
    ...
    p = Pool(threads_num)
    for ctg in pysam.VariantFile(vcf + ".gz").header.contigs:
        p.apply_async(filter_main, args=(f"{out_dir}/{ctg}/{os.path.split(vcf)[1]}.{ctg}", os.path.abspath(os.path.join(out_dir, ctg, "fragment_file")), f"{out_dir}/{ctg}/{os.path.split(vcf)[1]}.{ctg}.filtered", os.path.abspath(os.path.join(out_dir, ctg)), FILTER_SNP, ctg))
    p.close()
    p.join()
